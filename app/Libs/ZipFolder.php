...
class ZipFolder
{
    ...
    public function zip ($zipfile, $folder, $ignored = null) {
        ...
        if ($this->zip->open($zipfile, $this->zip::CREATE) !== true) {
        ...
        if(strstr($folder, DIRECTORY_SEPARATOR)) {
            $this->root = $folder.DIRECTORY_SEPARATOR;
        }
        $this->createZip();
     ...
     * @return void
     */
    private function createZip ($folder=NULL, $parent=null) {

        $full_path = $this->root . $parent . $folder;
        $zip_path = $parent.$folder;
        if( $folder || $parent){
            $this->zip->addEmptyDir($zip_path);
            $zip_path_val = $zip_path.DIRECTORY_SEPARATOR;
        }else{
            $zip_path_val = NULL;
        }
        ...
        foreach($dir as $file) {
            if(!$file->isDot()) {
                ...
                if(!in_array($filename, $this->ignored_names)) {
                    if($file->isDir()) {
                        $this->createZip($filename, $zip_path_val);
                    }else {
                        if($zip_path){
                            $this->zip->addFile($full_path.DIRECTORY_SEPARATOR.$filename, $zip_path.DIRECTORY_SEPARATOR.$filename);
                        }else{
                            $this->zip->addFile($full_path.DIRECTORY_SEPARATOR.$filename, $filename);
                        }
                    }
                }
            }
...
$zip->zip(__DIR__. DIRECTORY_SEPARATOR .time().'.zip', __DIR__. DIRECTORY_SEPARATOR .'demo');*/