...
function flag_html(config_flag, file) {
    str = '';
    if (config_flag == 'img') {

    } else {
        str = '<div id="' + $(item)[0].id + file.id + '" class="item">' +
...
// alert(pickerid);
    ...
    var config_flag = opts.datas.config_flag;
    var html_str = '';
    ...
    if (opts.auto) {
        ...
        uploader.on('fileQueued', function (file) {
            console.log(file);
            if (config_flag == 'img') {
                html_str = '<div id="' + $(item)[0].id + file.id + '" class="item image_list">' +
                    '<div><img src="" alt=""></div>' +
                ...
                html_str = '<div id="' + $(item)[0].id + file.id + '" class="item">' +
                    '<span class="webuploadinfo" onclick="show_file(\'' + options.name + '\')">' + file.name + '</span>' +
                    '<div class="webuploadinfodiv"><span class="webuploadsize">' + WebUploader.formatSize(file.size) + '</span>' +
                    '<span class="webuploadstate">正在上传...</span>' +
                    '<div class="webuploadDelbtn">删除</div></div>' +
    ...
    uploader.on('uploadSuccess', function (file, response) {//上传成功事件
        ...
        if (typeof(response.code) == 'undefined' || typeof(response.message) == 'undefined' || response.code < 0) {
            ...
            target.find('#' + $(item)[0].id + file.id).find('img').prop('src',response.file_info.full_path);
            target.find('#' + $(item)[0].id + file.id).find('.webuploadstate').html('上传成功');
            if (options.limit) {
