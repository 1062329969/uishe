...
function image_flexImages(block,rowHeight,flexTruncate) {
    ...
    if (block == null)
	{
		var belongs = $(".flex-images");
	}else {
        var belongs = block.find(".flex-images");
	}
    ...
    if(belongs.size() > 0 && belongs.find(".item").size() > 0)
    {
        ...
        belongs.find(".item").each(function(){
			...
			if($(this).width() < 250)
			{
				 $(this).find(".fav-star").remove();
			}
		
            if($(this).width() < 200)
            {
                $(this).find(".hide-top").css("top","-66px");

                $(this).hover(function(){
                    $(this).find(".hide-top").stop().animate({top:'10px'},"fast");
                    $(this).find(".hide-bottom").stop().animate({bottom:'0'},"fast");
                }, function () {
                    $(this).find(".hide-top").stop().animate({top:'-66px'},"fast");
                    $(this).find(".hide-bottom").stop().animate({bottom:'-66px'},"fast");
                });

                $(this).find(".psd-download").css("margin-top","10px");

            }else{

                $(this).hover(function(){
                    $(this).find(".hide-top,.fav-star").stop().animate({top:'10px'},"fast");
                    $(this).find(".hide-bottom").stop().animate({bottom:'0'},"fast");
                }, function () {
                    $(this).find(".hide-top,.fav-star").stop().animate({top:'-35px'},"fast");
                    $(this).find(".hide-bottom").stop().animate({bottom:'-35px'},"fast");
                });
            }
...
/*滚动条下滑固定头部*/
    $(window).scroll(function () {
		...
		if ($(".top-scroll").size() > 0) {
		   ...
		   if($(".top-fixed").size() > 0){
			   if($(document).scrollTop() > 60 && !$(".top").hasClass("fixeding")){
				   $(".top").addClass("fixeding").css({"width":"100%","position":"fixed","top":"-60px"}).animate({"left":"0px","top":"0px"},200);
    ...
    $(".new-fixed-button a").hover(function () {
...
/*头部搜索框导航菜单*/
   ...
   $(".search-result-box .search-keydown-href").live("click",function(event) {
        ...
        $.getJSON("/?m=Search&a=index&ajax=1&callback=?",{"keyword":keyword,"plate_path":plate_path},function(data){
            if(data.status == 1) {
    ...
    $(".banner-search").keyup(function(){
		return false;
        var keyword = $(this).val();
        if($(this).attr("placeholder") == keyword || keyword == ''){
            ...
            if (G != 38 && G != 40 && G != 13) {
                var num = 1;
                D = 0;
                if($('.banner-search').val() == ''){
                    //默认搜索词
                }else{
                    var plate_path = $(".search-category .on").attr("data-plate-path");
                    var word = $('.banner-search').val();
                    $('.search-result-box').children().remove();
                    $.ajax({
                        type: 'get',
                        url: '/?m=search&a=getThinkWords&ajax=1',
                        data: {"word": word,"plate_path":plate_path},
                        dataType: 'json',
                        success: function (rs) {
                            $('.search-result-box').html('');
                            var html = '';
                            $.each(rs,function(i,item) {
                                html += '<a href="javascript:;" class="block fn12 search-keydown-href" data-plate-path="'+plate_path+'" data-word="'+item.word+'"><div class="fl">'+item.word+'</div><div class="fr">'+item.search_result+'个结果</div></a>';
                            });
                            $('.search-result-box').append(html);
							$(".search-category-title").show();
   ...
   $(".picBox").hover(function() {
    ...
    $(".picBox").live("mouseover",function(){$(this).children(".pic-item-action").show();$(this).children(".title-content").hide();});
    $(".picBox").live("mouseleave",function(){$(this).children(".title-content").show();$(this).children(".pic-item-action").hide()});
...
/*点击展示版权*/ 
$(".copyright-list").live("click",function() {
	var picid = $(this).parent().find("a").attr("data-id");
...
$(".search-input,.tsearch-input").keydown(function(event){if (event.which == 13) { search(this);}});
...
function showSearchSelectBox(_this)
{
    ...
    var searchWidth  =  $(_this).attr('data-search-width')  ? $(_this).attr('data-search-width') : '640' ;
    ...
    var htmlAdvice = '<div class="f1 search-advice none" style=" width:' + searchWidth + 'px; margin-left: '+searchLeft+'px; top: '+searchTop+'px;"><ul>' ;
        ...
        if (_is_relase_search) 
        {
            ...
            '<a href="javascript:;" class="search-submit-adv" onclick="changeOnSearch(this)" data-plate-path="ppt" style="width: '+a_width+'px;">搜PPT</a>' +
            '<a href="javascript:;" class="search-submit-adv" onclick="changeOnSearch(this)" data-plate-path="muban">搜平面模板</span></a>' +
            '<a href="javascript:;" class="search-submit-adv" onclick="changeOnSearch(this)" data-plate-path="png">搜PNG元素</a>' +
            '<a href="javascript:;" class="search-submit-adv" onclick="changeOnSearch(this)" data-plate-path="beijing" style="width: '+a_width+'px;">搜背景</a>' +
            '<a href="javascript:;" class="search-submit-adv" onclick="changeOnSearch(this)" data-plate-path="tupian" style="width: '+a_width+'px;">搜图片</a>' +
            '</li>';
        }
...
$('.search-input, .tsearch-input').live('input propertychange',function () {
    showSearchSelectBox(this);
});

$('.search-input, .tsearch-input').click(function () {
    showSearchSelectBox(this);
});
...
function changeOnSearch(_this) {
	...
	if($('.search-input').is(':hidden')){　　//如果node是隐藏的则显示node元素，否则隐藏

　　	var keyword	= $('.tsearch-input').val();
